...
class provider(object):
    ...
    def __init__(self, for_what, batch_size=1, whether_aug=False):
        ...
        if for_what not in ["train", "predict", "evaluate"]:
            ...
            if for_what == "train":
                ...
                # Get sensor data
                self.sensor_token = self.nusc.get_sensor_token_list(self.night_scene[0])
    ...
    def __send_data(self):
        ...
        while True:
            ...
            sensor_token_pick = random.sample(self.sensor_token, 1)[0]
            im = ip(sensor_token_pick, self.nusc, mini_size=self.__mini_size)
            ## resize img and normalize img and bboxes##
            if self.__for_what == "train":
                cell_class, corner_point_deta = im.annotations_in_net()
                self.__data_queue.put([im.image, cell_class, corner_point_deta])
            else:
...
if __name__ == '__main__':
    dt = provider(batch_size=5, for_what="train")

    for step in range(2):
        imgs, labels, control_points = dt.load_batch()
        if step%10 == 0:
            logger.info('Time')
        ## do sth ##