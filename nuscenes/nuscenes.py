...
class NuScenes:
    ...
    def list_night(self) -> List[str]:
        return self.explorer.list_night()
...
class NuScenesExplorer:
    ...
    def list_night(self) -> List[str]:
        ...
        for start_time, record in sorted(recs):
            ...
            if 'Night' in desc:
                night_scene.append(desc.split(',')[0])

        return night_scene

    def get_sensor_token_list(self, scene, sensor='CAM_FRONT'):
        my_scene_token = self.nusc.field2token('scene', 'name', scene)[0]
        sce_rec = self.nusc.get('scene', my_scene_token)
        sample_rec = self.nusc.get('sample', sce_rec['first_sample_token'])
        sensor_token = []
        for i_sample in range(sce_rec['nbr_samples']):
            sensor_token.append(sample_rec['data'][sensor])
